...
interface EspoApiService {
    ...
    suspend fun searchCalls(
        @Query("where[0][type]") type0: String = "equals",
        @Query("where[0][attribute]") attr0: String = "dateStart",
        @Query("where[0][value]") dateStart: String,
        @Query("where[1][type]") type1: String = "equals",
        @Query("where[1][attribute]") attr1: String = "cUserPhone",
        @Query("where[1][value]") cUserPhone: String
    ): Response<EspoSearchResponse>
}

data class EspoSearchResponse(
    val total: Int = 0,
    val list: List<EspoCallResponse> = emptyList()
)
...
data class EspoCallRequest(
    ...
    val cContactName: String? = null,
    val cCallType: String? = null,
